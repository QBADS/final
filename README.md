...
| IBM Quantum job management (`services/quantum-pipeline`'s `app/providers/`, `services/middleware`'s `/api/v1/quantum/*`) | Built and tested — a real `QuantumProvider` abstraction (`IBMQuantumProvider`/`SimulatorQuantumProvider`/`MockQuantumProvider`) plus a versioned, exec-admin-only, auth/idempotency/rate-limit-protected job-submission API. Deliberately separate from the fraud-scoring path above (see `services/middleware/README.md`) — not run against a live IBM account in this environment; validated via 35 pytest + 27 Vitest tests, all mocked HTTP, plus a manual end-to-end curl walkthrough against the real running services in mock-provider mode | n/a (new capability, no source spec doc) |
